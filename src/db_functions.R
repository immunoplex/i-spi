...
pull_data_frequentist <- function(study_accession, experiment_accession, project_id, curve_id_elements, conn = conn) {
  plates <- fetch_db_header(study_accession = study_accession,
                            experiment_accession = experiment_accession,
                            project_id = project_id,
                            conn = conn)
...
get_curve_ids <- function(conn, study_accession, project_id, experiment_accession) {
  
  query <- glue::glue_sql("
    SELECT curve_id, project_id, study_accession, experiment_accession, plateid, 
    plate, nominal_sample_dilution, feature, antigen, source, wavelength
...
swap_curve_id <- function(df, curve_id_lookup,
  ...
  # Reconstruct source_nom exactly as pull_data_frequentist built the composite key:
  # "source|wavelength_nm" when a real wavelength exists, plain source otherwise.
  lk$source_nom <- ifelse(
    !is.na(lk$wavelength) & lk$wavelength != "__none__",
    paste0(lk$source, "|", lk$wavelength, "_nm"),
    as.character(lk$source)
  )
  
  # Rebuild the composite string key from the lookup columns
  lk$composite_key <- paste(
    lk$project_id,
    lk$study_accession,
    lk$experiment_accession,
    lk$feature,
    lk$source_nom,
    lk$antigen,
    lk$plate,
    lk$nominal_sample_dilution,
    lk$wavelength,
    sep = ":"
  )
  
  # Named vector: composite_key → integer curve_id
  key_to_id <- setNames(lk[[curve_id_col]], lk$composite_key)
  
  # Swap
  old_keys <- as.character(df[[curve_id_col]])
  new_ids  <- unname(key_to_id[old_keys])
  
  if (verbose) {
    n_miss <- sum(is.na(new_ids))
    if (n_miss > 0)
      message(sprintf(
        "[swap_curve_id] %d / %d rows unmatched — sample composite keys not found in lookup",
        n_miss, nrow(df)
      ))
    else
      message(sprintf("[swap_curve_id] All %d rows matched.", nrow(df)))
  }
  
  df[[curve_id_col]] <- new_ids
  df
}
# swap_curve_id <- function(df, curve_id_lookup) {
#   
